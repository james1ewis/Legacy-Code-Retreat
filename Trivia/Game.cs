...
namespace UglyTrivia
{
    public class Game
    {
        ...
        {
            ConsoleWriteLine(players[currentPlayer] + " is the current player");
            ...
            if (inPenaltyBox[currentPlayer])
            {
                if (roll % 2 != 0)
                {
                    ...
                    ConsoleWriteLine(players[currentPlayer] + " is getting out of the penalty box");
                    places[currentPlayer] = places[currentPlayer] + roll;
                    if (places[currentPlayer] > 11) places[currentPlayer] = places[currentPlayer] - 12;

                    ConsoleWriteLine(players[currentPlayer]
                            + "'s new location is "
                ...
                {
                    ConsoleWriteLine(players[currentPlayer] + " is not getting out of the penalty box");
        ...
        private void askQuestion()
        {
            ...
            {
                ConsoleWriteLine(popQuestions.First());
                popQuestions.RemoveFirst();
            }
            if (currentCategory() == "Science")
            {
                ConsoleWriteLine(scienceQuestions.First());
                scienceQuestions.RemoveFirst();
            }
            if (currentCategory() == "Sports")
            {
                ConsoleWriteLine(sportsQuestions.First());
                sportsQuestions.RemoveFirst();
            }
            if (currentCategory() == "Rock")
            {
                ConsoleWriteLine(rockQuestions.First());
